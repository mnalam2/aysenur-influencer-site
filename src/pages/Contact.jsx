      ...
      <div className="max-w-4xl mx-auto px-4 sm:px-6 py-20">
        <div className="grid sm:grid-cols-3 gap-6 mb-10">
          {[
            ...
            <motion.div key={c.label} {...fi(0.05)}>
              <GlassCard className="p-7 text-center flex flex-col items-center h-full transition-colors">
                <div className="w-12 h-12 rounded-2xl flex items-center justify-center mb-4"
                  style={{ background: "rgba(239,65,54,0.08)", border: `1px solid rgba(239,65,54,0.18)`, color: RED }}>
                  {c.icon}
                </div>
                <div className="text-xs font-bold uppercase tracking-widest mb-2" style={{ color: MUTED2 }}>{c.label}</div>
                {c.href
                  ? <a href={c.href} className="font-semibold text-sm transition-colors" style={{ color: TEXT }}>{c.value}</a>
                  : <p className="font-semibold text-sm whitespace-pre-line text-center" style={{ color: TEXT }}>{c.value}</p>}
              </GlassCard>
            </motion.div>
          ))}
        </div>
        ...
        <motion.div {...fi(0.1)}>
          <GlassCard className="p-8 text-center mb-6">
            <h3 className="font-bold text-xl mb-3" style={{ color: TEXT }}>Send Us an Email</h3>
            <p className="text-sm mb-6" style={{ color: MUTED }}>All orders and inquiries handled personally.</p>
        ...
        <motion.div {...fi(0.15)}>
          <GlassCard className="p-8 flex flex-col sm:flex-row items-center justify-between gap-6"
            ...
            <div>
              ...
              <h3 className="font-bold text-xl mb-2" style={{ color: TEXT }}>Join the MOVI Team</h3>
            ...
            <a href="mailto:work@moviphones.com?subject=Career%20Inquiry&body=Hi%20WMS%20Team%2C%0A%0AI%27d%20love%20to%20join%20your%20team.%0A%0AName%3A%20%0ARole%20interest%3A%20"
              ...
              style={{ background: "rgba(239,65,54,0.08)", border: `1px solid rgba(239,65,54,0.25)`, color: RED }}>
